...
async function init() {
  ...
  if (!adminRows.length || !adminRows[0].values.length) {
    ...
    db.run('INSERT INTO admins (username, password, name) VALUES (?, ?, ?)', ['admin', hash, 'ผู้ดูแลระบบ']);
  }

  // Seed products
  const cntRows = db.exec('SELECT COUNT(*) as cnt FROM products');
  const cnt = cntRows[0].values[0][0];
  if (cnt === 0) {
    const products = [
    ...
    for (const p of products) {
      db.run('INSERT INTO products (name,description,price,unit,stock,image_url,category) VALUES (?,?,?,?,?,?,?)', p);
    }
  }
  save();
  return { db, save };
